...
df_2_in <- tibble::tibble(time = c(100, 200, 300),
                          event = c(1, 0, 1),
                          n_events = c(1, 2, 2))
...
testthat::test_that("Default Estimator", {
  ...
  expect_equal(mr_regression(in_data = df_2_in, time = "time", event = "event", n_events = "n_events", simplified = FALSE)$F_i,
               expected = df_2_exp$F_i_default)
})
...
testthat::test_that("Simplified Estimator", {
  ...
  expect_equal(mr_regression(in_data = df_2_in, time = "time", event = "event", n_events = "n_events", simplified = TRUE)$F_i,
               expected = df_2_exp$F_i_simplified)
})
